...
EditorWindow::EditorWindow(myClient *client, QWidget *parent) : QMainWindow(parent), ui(new Ui::EditorWindow),
                                                                _client(client) {
    ui->setupUi(this);
...
void EditorWindow::LoginProcedure(){
    QString user = "user1";
    QByteArray ba_user = user.toLocal8Bit();
    const char *c_user = ba_user.data();
    ...
    //Serialize data
    json j;
    jsonUtility::to_json(j, "LOGIN_REQUEST", c_user);
    const std::string req = j.dump();
